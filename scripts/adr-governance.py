...
# Source: WithAutonomi/ant-node at 1e9dc01ffbd896f8c0c453808e87cfd73d688e8c,
# via WithAutonomi/adr-workbench at 88f34671f0bd1992b086ee8216eca2787c71071a.
# Modified only to add licensing/provenance comments. See NOTICE in this source
# distribution; installed as docs/adr/NOTICE.
"""Repository-local ADR governance checks (best-effort; see docs/adr/README.md
for the gate's exact scope and limits).

Checks, on the ADR files it discovers (those matching the glob ADR-*.md; the
stricter ADR-NNNN-short-title.md convention is then validated as one of the checks):
- filename convention; required sections present; status present and one of the
  allowed lifecycle *values* (not transitions).
- duplicate ADR numbers, across all discovered ADRs.
- Accepted ADRs should not be edited after acceptance (create a superseding ADR).
  Checked only for changed ADRs when a base commit is available; a git-diff error
  fails open. Files not named ADR-*.md are not discovered; human identity is not
  enforced.
"""
